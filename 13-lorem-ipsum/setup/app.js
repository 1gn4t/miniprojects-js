...
// create paragraph
function getParagraph(arr) {
  const randomNum = Math.floor(Math.random() * text.length);
  let elem = document.createElement("p");
  elem.className = "result";
  elem.textContent = arr[randomNum];
  return elem;
}

// create paragraphs of array
function getListParagraphs(value) {
  return [...Array(value)].map(() => getParagraph(text));
}
...
function addParagraphs() {
  ...
  if (isNaN(value) || value < 0 || value > 10) {
    result.innerHTML = "";
    result.append(
      "Enter the number of paragraphs no more than 10 and no less than 1"
    );
  } else {
    result.innerHTML = "";
    result.append(...getListParagraphs(value));
  }
}
